...
# def facegrid_more(train_df,target,Age,Pclass):
#     # grid = sns.FacetGrid(train_df, col='Pclass', hue=target)
#     grid = sns.FacetGrid(train_df, col=target, row=Pclass, size=2.2, aspect=1.6)
#     grid.map(plt.hist, Age, alpha=.5, bins=20)
#     grid.add_legend();
    
# def facegrid_pointplot(train_df,Embarked,Pclass, target,Sex):
#     # grid = sns.FacetGrid(train_df, col='Embarked')
#     grid = sns.FacetGrid(train_df, row=Embarked, size=2.2, aspect=1.6)
#     grid.map(sns.pointplot, Pclass, target,Sex, palette='deep')
#     grid.add_legend()
